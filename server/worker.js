...
export default {
  async fetch(request, env, ctx) {
    ...
    // Fetch-запросы из service worker'а расширения к хосту, разрешённому через
    // host_permissions, на практике часто идут БЕЗ заголовка Origin вообще
    // (Chrome не обязан его слать для таких привилегированных запросов) — так
    // что отсутствие Origin разрешаем, а вот подделанный под чужую страницу
    // Origin (реальный fetch из JS на произвольном сайте всегда его проставляет)
    // отклоняем. Не криптографическая защита, а первый фильтр от случайного трафика.
